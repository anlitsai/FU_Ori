...
#dir_obj='slt_InstMag_'+filter_ID+'mag_AAVOS/'
dir_obj='slt_InstMag_'+filter_ID+'mag_VRIphot/'
...
#file_refstar='slt_refStar_radec_annu_AAVOS.txt'
#file_refstar='slt_refStar_radec_annu_VRIphot.txt'
file_refstar='slt_refStar_radec_annu_VRIphot_ACDEI.txt'

df_refstar=pd.read_csv(file_refstar,sep='|')
#print(df_refstar)

if len(sys.argv) > 1:
    refstar_skip_ID=sys.argv[1]
    idx_refstar_skip = df_refstar[ df_refstar['RefStarID'] == refstar_skip_ID ].index
    df_refstar.drop(idx_refstar_skip, inplace = True)
...
date_obs=df_info['DateObs'][idx_fitsheader]
#print(date_obs)
...
k=0
print("n_elements(idx_fitsheader_canfit) = ", len(idx_fitsheader_canfit))
print("n_idx = ", n_idx)

for i in idx_fitsheader_canfit:
    print('=================================')   
    print('idx',i, ') ID =',ID[i],', #', k+1,'/',n_idx,'image of',obj_name,filter_ID,'band')
...
#    This method provides a better estimate of the background and background noise levels:
    ...
    for j1 in range(n_refstar):
        print('idx',i, '-',j1+1,'/',n_refstar,', Ref.Star:',refstarID[j1],') #', k+1,'/',n_idx,'image of',obj_name,filter_ID,'band' )
...
#    param,res=polyfit(mag_instrument[k][1:],vmag[1:],1,full=True)
    ...
    plt.xlim(x1,x2)
    plt.ylim(y1,y2)
    plt.xlabel('relative RA (J2000)',fontsize=font_small)
    plt.ylabel('relative Dec (J2000)',fontsize=font_small)
...
df_out['InstMag_I']=mag_instrument[:,5]
df_out['InstMag_I']=df_out['InstMag_I'].map('{:.4f}'.format)
