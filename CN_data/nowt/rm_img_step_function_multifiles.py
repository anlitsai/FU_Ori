...
path_flatten='nowt/bg_flatten/'
if os.path.exists(path_flatten):
    shutil.rmtree(path_flatten)
os.makedirs(path_flatten,exist_ok=True)



cmd_search_file='find ./nowt | grep Ori | grep fits'
print(cmd_search_file)
list_file=os.popen(cmd_search_file,"r").read().splitlines()
print(list_file)
...
for i in list_file:
    print("filename: ",i)
    file_root=i.split('/',-1)[-1].split('.',-1)[0]
...
#plt.show()
    ...
    path_file_bg_flatten=path_flatten+file_root+'_bg_flatten.fits'
